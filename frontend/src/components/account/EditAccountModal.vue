...
function buildUpstreamCostProfilesPayload(): UpstreamCostProfileInput[] {
  if (upstreamCostProfiles.value.length === 0) {
    // 编辑场景必须显式发送空数组，后端据此区分“清空价格档案”与
    // “本次没有编辑价格”（字段缺失 / nil）。
    return []
  }
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    if (newAccount.type === 'oauth') {
      ...
      // 缺省/非法值按 session（设备+会话）呈现，与后端默认语义一致。
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      const rateSyncWasEnabled =
        props.account.extra?.upstream_billing_probe_enabled === true &&
        props.account.extra?.upstream_billing_rate_sync_enabled === true
      const rateSyncWasDisabledByThisEdit =
        rateSyncWasEnabled && !upstreamBillingRateSyncEnabled.value
      if (!rateMultiplierTouched.value && !rateSyncWasDisabledByThisEdit) {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      // 指纹收敛默认 session；始终落键，确保显式选择 off 不会被缺省语义覆盖。
      if (props.account.type === 'oauth') {
