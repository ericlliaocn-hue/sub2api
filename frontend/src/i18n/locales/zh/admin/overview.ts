export default {
    ...
    users: {
      ...
      bonusGrant: {
        ...
        title: '赠送限时余额',
        selectedCount: '已选择 {count} 个用户',
        atomicHint: '本次发放全量成功或全量失败；重复提交不会重复到账。',
        amount: '每位用户赠送金额',
        expiresAt: '统一到期时间',
        campaignId: '活动批次',
        campaignPlaceholder: '例如 weekend-20260830',
        totalGranted: '本次赠送总额',
        expiryWarning: '到期后未使用的赠送余额会自动收回，请再次确认时间。',
        confirm: '确认赠送',
        granting: '发放中...',
        confirmMessage: '确定向 {count} 个用户每人赠送 ${amount} 吗？\n总额：${total}\n到期：{expiresAt}',
        success: '已向 {count} 个用户发放限时余额',
        failed: '赠送余额发放失败'
      },
      editUser: '编辑用户',
      deleteUser: '删除用户',
      ...
      platformOther: '其他',
      balanceHistoryTitle: '用户余额及权益变动记录',
      balanceHistoryCredits: '充值兑换及人工正向调整合计',
      balanceHistoryLoadFailed: '变动记录加载失败',
      historyEvents: {
        registration_bonus: '注册赠送', initial_balance: '开户初始余额',
        admin_bonus_granted: '管理员活动赠送', recharge_bonus_granted: '充值活动赠送',
        bonus_expired: '赠送余额到期', payment_refund: '支付退款',
        bonus_expiry_changed: '赠送有效期调整', subscription_created: '订阅开通',
        subscription_changed: '订阅变更', subscription_removed: '订阅移除',
      },
    ...
    groups: {
      ...
      subPools: {
        ...
        graduation: {
          title: '观察期毕业规则',
          hint: '全局生效。新 Key 按「用户指定 → 分组默认 → 观察池」进池。满足观察期且期间无违规、用量未超阈值后，自动迁入负载最低的正式池。已指定用户不会被毕业带走。关闭时不会自动迁任何 Key。',
          probationDays: '观察期（天）',
        ...
        title: '子池隔离',
        enableLabel: '启用子池隔离',
        enableHint: '开启后按用户进池：指定用户的所有 Key 和新 Key 只打自己的池，其他人走分组默认。用户侧看不到子池。',
        enabled: '已启用',
        disabled: '未启用',
        disabledHint: '该分组尚未启用子池隔离，下面的配置暂不会影响调度。可在编辑分组时打开开关。',
        groupAccounts: '分组账号 {count} 个',
        createPool: '新建子池',
        namePlaceholder: '子池名称',
        keySoftLimitHint: 'Key 软上限，0 表示不限',
        empty: '还没有子池',
        poolSummary: 'Key {keys}/{limit} · 账号 {accounts}',
        kinds: {
          formal: '正式池',
          probe: '观察池'
        },
        ...
        routing: {
          title: '谁打哪个号',
          hint: '按用户进池，他的 Key 和新 Key 都跟着走。只有某一把脏了才单独改 Key。',
          user: '用户',
          key: 'Key',
          keys: 'Key 数',
          pool: '子池',
        ...
        groupDefault: '其他人默认进',
        groupDefaultHint: '没单独指定的用户，新建 Key 进这个池。已有 Key 不会被自动搬走。',
        groupDefaultNone: '不指定（仍按观察池优先）',
        placeUser: '把用户放进此池',
        placeUserPlaceholder: '选择用户',
        placeUserSuccess: '已把 {user} 的 Key 全部放进「{pool}」',
        placeUserFailed: '设置用户子池失败',
        saveGroupDefaultFailed: '保存分组默认池失败',
        clearUserPin: '取消指定',
        clearUserPinSuccess: '已取消 {user} 的指定，现有 Key 仍留在原池',
        clearUserPinFailed: '取消指定失败',
        mixedKeys: 'Key 不在同一池',
        boundUsers: '池内用户',
        noBoundUsers: '还没有用户进这个池',
        boundKeys: '已绑 Key',
        noBoundKeys: '还没有 Key 绑到这个池',
        overrideKey: '单把 Key 例外',
        overrideKeyHint: '只有某一把脏了才改这一项；平时按上面的用户进池。',
        bindKey: '把 Key 绑到此池',
        bindKeyPlaceholder: '选择用户 / Key',
        bindKeySuccess: '已把 Key #{id} 绑到「{pool}」',
        bindKeyFailed: '绑定 Key 失败',
        reachableNow: '能打到',
        schedulableOn: '调度开',
        schedulableOff: '调度关',
        ping: 'Ping',
        lastUsed: '上次用量',
        editAccounts: '账号',
        pickAccounts: '选择该子池使用的账号',
        noGroupAccounts: '该分组下没有可用账号',
        accountTaken: '已被其他子池占用',
        accountShared: '和其他子池共用',
        migrate: '迁走清白 Key',
        migrateHint: '把没有嫌疑的 Key 迁到其他子池；填写的嫌疑 Key 会留在本池，便于继续取证。',
        migrateReasonPlaceholder: '迁移原因，例如：pool_burned',
        suspectKeysPlaceholder: '嫌疑 Key ID，逗号分隔（留空表示全部迁走）',
        migrateSuccess: '已迁走 {count} 个 Key',
        cool: '冷却',
        resume: '恢复',
        deleteTitle: '删除子池',
        deleteMessage: '删除子池「{name}」后，池内 Key 将回退到整组调度。确认删除？',
        loadFailed: '加载子池失败',
        createFailed: '创建子池失败',
        saveFailed: '保存子池失败',
        saveAccountsFailed: '保存子池账号失败',
        migrateFailed: '迁移失败',
        deleteFailed: '删除子池失败'
      },
