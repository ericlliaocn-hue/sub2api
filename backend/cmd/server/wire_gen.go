...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	subPoolRepository := repository.NewSubPoolRepository(client, db)
	subPoolUsageRepository := repository.NewSubPoolUsageRepository(client, db)
	...
	subPoolMembership := service.ProvideSubPoolMembership(subPoolRepository, gatewayService, openAIGatewayService, geminiMessagesCompatService)
	...
	adminService := service.ProvideAdminService(configConfig, userRepository, adminGroupRepository, adminAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, affiliateService, compositeModelRouteRepository, compositeRouteResolver, channelService, promotionService)
	subPoolService := service.ProvideSubPoolService(subPoolRepository, groupRepository, apiKeyRepository, subPoolUsageRepository, subPoolMembership, apiKeyService, adminService)
	subPoolUserHandler := handler.NewSubPoolUserHandler(subPoolService)
	...
	watchedTrafficRepository := repository.NewWatchedTrafficRepository(db)
	watchedTrafficService := service.NewWatchedTrafficService(settingRepository, watchedTrafficRepository)
	watchedTrafficHandler := admin.NewWatchedTrafficHandler(watchedTrafficService)
	...
	postgreSQLRuleRepository := securityaudit.NewPostgreSQLRuleRepository(db)
	promptService := securityaudit.NewPromptServiceWithRules(configManager, postgreSQLRepository, redisPayloadStore, openAICompatibleScanner, atomicMetrics, postgreSQLRuleRepository)
	...
	creationHistoryService := service.NewCreationHistoryService(db, configConfig)
	creationHistoryHandler := admin.NewCreationHistoryHandler(creationHistoryService)
	businessFinanceRepository := repository.NewBusinessFinanceRepository(db)
	businessFinanceService := service.NewBusinessFinanceService(businessFinanceRepository)
	businessFinanceHandler := admin.NewBusinessFinanceHandler(businessFinanceService)
	promotionHandler := admin.NewPromotionHandler(promotionService)
	upstreamConnectionService := service.NewUpstreamConnectionService(settingRepository, secretEncryptor)
	upstreamConnectionHandler := admin.NewUpstreamConnectionHandler(upstreamConnectionService)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, subPoolHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, watchedTrafficHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService, creationHistoryHandler, businessFinanceHandler, promotionHandler, upstreamConnectionHandler)
	...
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, creationHistoryService, configConfig, coordinator)
	...
	handlerCreationHistoryHandler := handler.NewCreationHistoryHandler(creationHistoryService, apiKeyService, settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, subPoolUserHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, handlerCreationHistoryHandler, watchedTrafficService, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	...
	paymentOrderExpiryService := service.ProvidePaymentOrderExpiryService(paymentService, billingCacheService, apiKeyAuthCacheInvalidator, leaderLockCache, db)
