    ...
    <div class="space-y-6 p-6">
      ...
      <section v-if="growth" class="card p-5">
        ...
        <div class="mt-5 grid gap-4 xl:grid-cols-2">
          <div class="overflow-x-auto"><p class="mb-2 text-xs text-gray-500">获客来源（用户从哪来，与推广报表同口径）</p><table class="min-w-full text-left text-sm"><thead class="border-b border-gray-200 text-xs text-gray-500 dark:border-dark-700"><tr><th class="px-3 py-2">来源</th><th class="px-3 py-2">拉新</th><th class="px-3 py-2">活跃</th><th class="px-3 py-2">付费</th><th class="px-3 py-2">消耗收入</th><th class="px-3 py-2">充值</th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-dark-700"><tr v-for="item in growth.by_source" :key="item.source"><td class="px-3 py-2"><span class="rounded px-1.5 py-0.5 text-[10px] font-medium" :class="acquisitionClassBadge(item.source)">{{ acquisitionClassLabel(item.source) }}</span></td><td class="px-3 py-2">{{ item.new_users }}</td><td class="px-3 py-2">{{ item.active_users }}</td><td class="px-3 py-2">{{ item.paying_users }}</td><td class="px-3 py-2">{{ money(item.revenue) }}</td><td class="px-3 py-2">{{ money(item.recharge) }}</td></tr></tbody></table></div>
          <div class="overflow-x-auto"><p class="mb-2 text-xs text-gray-500">注册方式（邮箱 / GitHub / LinuxDo 等登录手段，不是来源）</p><table class="min-w-full text-left text-sm"><thead class="border-b border-gray-200 text-xs text-gray-500 dark:border-dark-700"><tr><th class="px-3 py-2">方式</th><th class="px-3 py-2">拉新</th><th class="px-3 py-2">活跃</th><th class="px-3 py-2">付费</th><th class="px-3 py-2">消耗收入</th><th class="px-3 py-2">充值</th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-dark-700"><tr v-for="item in growth.by_signup_method || []" :key="item.source"><td class="px-3 py-2 font-mono text-xs">{{ item.source }}</td><td class="px-3 py-2">{{ item.new_users }}</td><td class="px-3 py-2">{{ item.active_users }}</td><td class="px-3 py-2">{{ item.paying_users }}</td><td class="px-3 py-2">{{ money(item.revenue) }}</td><td class="px-3 py-2">{{ money(item.recharge) }}</td></tr><tr v-if="!growth.by_signup_method?.length"><td colspan="6" class="px-3 py-4 text-center text-xs text-gray-500">暂无数据</td></tr></tbody></table></div>
        </div>
      </section>
    </div>
  </AppLayout>
