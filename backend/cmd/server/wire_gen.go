...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	adminCreationHistoryHandler := admin.NewCreationHistoryHandler(creationHistoryService)
	...
	creationHistoryHandler := handler.NewCreationHistoryHandler(creationHistoryService, apiKeyService)
	creationHistoryHandler.SetSettingService(settingService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, creationHistoryHandler, idempotencyCoordinator, idempotencyCleanupService)
