...
type APIKey struct {
	...
	// Sub-pool this key is bound to; NULL keeps whole-group scheduling
...
// was not loaded in eager-loading, or loaded but was not found.
func (e APIKeyEdges) SubPoolOrErr() (*SubPool, error) {
	if e.SubPool != nil {
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldID, apikey.FieldUserID, apikey.FieldGroupID, apikey.FieldSubPoolID:
...
// QuerySubPool queries the "sub_pool" edge of the APIKey entity.
func (_m *APIKey) QuerySubPool() *SubPoolQuery {
	return NewAPIKeyClient(_m.config).QuerySubPool(_m)
}
