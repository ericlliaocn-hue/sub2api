...
type Group struct {
	...
	// 是否让此分组的 API Key 走子池调度，而不是整组调度
	SubPoolEnabled bool `json:"sub_pool_enabled,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldSubPoolEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field sub_pool_enabled", values[i])
			} else if value.Valid {
				_m.SubPoolEnabled = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("sub_pool_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.SubPoolEnabled))
	builder.WriteString(", ")
