...
func RegisterGatewayRoutes(
	...
	watchedTraffic := handler.WatchedTrafficMiddleware(h.WatchedTraffic)
	...
	videoGenerationHandler := func(c *gin.Context) {
		...
		if platform := getGroupPlatform(c); platform == service.PlatformGrok || platform == service.PlatformSeedance || platform == service.PlatformComposite {
	...
	videoStatusHandler := func(c *gin.Context) {
		...
		if getGroupPlatform(c) == service.PlatformGrok || getGroupPlatform(c) == service.PlatformSeedance || getGroupPlatform(c) == service.PlatformComposite {
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), watchedTraffic, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), watchedTraffic, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	// Antigravity 模型列表
	r.GET("/antigravity/models", gin.HandlerFunc(apiKeyAuth), watchedTraffic, requireGroupAnthropic, h.Gateway.AntigravityModels)
	...
	antigravityV1.Use(watchedTraffic)
	...
	antigravityV1Beta.Use(watchedTraffic)
