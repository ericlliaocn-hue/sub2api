export default {
    settings: {
      ...
      payment: {
        ...
        rechargeBonusTiers: '充值活动档位',
        rechargeBonusTiersHint: '仅当用户支付金额与档位完全一致时赠送，赠送 Token 不计入实付、手续费和邀请返利基数。',
        addRechargeBonusTier: '新增档位',
        bonusCurrency: '支付币种',
        bonusPaymentAmount: '支付金额',
        bonusAmount: '赠送 $ Token',
        bonusEnabled: '启用',
        bonusExpiryMode: '活动结束方式',
        bonusExpiryByDays: '启用后持续指定天数',
        bonusExpiryFixed: '指定到期时间',
        bonusDurationDays: '活动持续时间（天）',
        bonusEndsAt: '活动到期时间',
        bonusExpiryHint: '到期判断由后端执行；时间到了会自动停止赠送，普通保存不会重置按天倒计时。',
        noRechargeBonusTiers: '暂无充值活动档位',
        ...
        field_sysId: '系统号（sys_id）',
        field_huifuId: '汇付商户号（huifu_id）',
        field_productId: '产品号（product_id）',
        field_projectId: '收银台项目 ID（project_id）',
        field_projectTitle: '收银台项目标题',
        field_skillSource: '接口来源标识',
        field_huifuApiBaseHint: '生产环境使用 https://api.huifu.com；联调环境使用 https://spin-test.cloudpnr.com。',
        field_huifuSkillSourceHint: '写入斗拱请求头 jpt-x-skill-source，通常保留默认 sub2api。',
        ...
        huifuGuideSummary: '汇付斗拱通过 PC/H5 托管收银台完成支付；桌面端使用 PC 收银台，移动端使用 H5 收银台。',
        huifuGuideNote: '页面只配置 sys_id、huifu_id、product_id 和 project_id。商户私钥与汇付公钥必须由服务器只读密钥文件提供，不进入页面或数据库。异步通知地址由系统生成，支付成功以验签后的异步通知或主动查询结果为准。退款默认关闭。',
        limitsTitle: '限额配置',
        limitSingleMin: '单笔最低',
        limitSingleMax: '单笔最高',
        limitDaily: '每日限额',
      ...
      user_pressure_view: {
        label: '允许用户查看现在压力',
        description: '开启后，用户可在用量页看到号池现场冷热和人数，不会看到其他用户或具体账号。默认仅管理员可见。',
      },
      saveSettings: '保存设置',
      saving: '保存中...',
