export default {
    ...
    users: {
      ...
      bonusGrant: {
        action: 'Grant bonus ({count})',
        singleAction: 'Grant expiring balance',
        title: 'Grant expiring balance',
        selectedCount: '{count} users selected',
        atomicHint: 'The whole batch succeeds or fails together. Retrying will not grant twice.',
        amount: 'Amount per user',
        expiresAt: 'Shared expiration time',
        campaignId: 'Campaign ID',
        campaignPlaceholder: 'For example, weekend-20260830',
        totalGranted: 'Total grant amount',
        expiryWarning: 'Unused promotional balance is reclaimed after expiration. Verify the time carefully.',
        confirm: 'Confirm grant',
        granting: 'Granting...',
        confirmMessage: 'Grant ${amount} to each of {count} users?\nTotal: ${total}\nExpires: {expiresAt}',
        success: 'Granted expiring balance to {count} users',
        failed: 'Failed to grant expiring balance'
      },
      editUser: 'Edit User',
      ...
      platformOther: 'Other',
      balanceHistoryTitle: 'Balance and Entitlement History',
      balanceHistoryCredits: 'Redemptions and Positive Admin Adjustments',
      balanceHistoryLoadFailed: 'Could not load change history',
      historyEvents: {
        registration_bonus: 'Registration bonus', initial_balance: 'Opening balance',
        admin_bonus_granted: 'Admin bonus', recharge_bonus_granted: 'Recharge bonus',
        bonus_expired: 'Bonus expired', payment_refund: 'Payment refund',
        bonus_expiry_changed: 'Bonus expiry changed', subscription_created: 'Subscription created',
        subscription_changed: 'Subscription changed', subscription_removed: 'Subscription removed',
      },
    ...
    groups: {
      ...
      subPools: {
        ...
        graduation: {
          ...
          hint: 'Applies globally. New keys follow user pin, then group default, then probe. After probation with no violations and usage below the cap they move to the emptiest formal pool. Pinned users are never graduated. Nothing moves automatically while this is off.',
        ...
        enableHint: 'When enabled, place users into a pool: their keys stay there. Everyone else follows the group default. Users never see sub-pools.',
        enabled: 'Enabled',
        disabled: 'Disabled',
        disabledHint: 'This group has sub-pool isolation turned off, so the settings below do not affect scheduling yet. Turn it on when editing the group.',
        groupAccounts: '{count} group accounts',
        createPool: 'New sub-pool',
        namePlaceholder: 'Sub-pool name',
        keySoftLimitHint: 'Key soft limit, 0 means unlimited',
        empty: 'No sub-pools yet',
        poolSummary: 'Keys {keys}/{limit} · Accounts {accounts}',
        kinds: {
          formal: 'Formal',
          probe: 'Probe'
        },
        ...
        routing: {
          title: 'Who reaches which account',
          hint: 'Place users into a pool. Their existing and new keys follow. Only override a single key when that key is dirty.',
          user: 'User',
          key: 'Key',
          keys: 'Keys',
          pool: 'Sub-pool',
          reachable: 'Can reach',
          wholeGroup: 'Whole group (unbound)',
          none: 'No schedulable account',
          empty: 'This group has no keys yet',
          pinned: 'Pinned',
          inherit: 'Group default'
        },
        groupDefault: 'Everyone else defaults to',
        groupDefaultHint: 'New keys for users without a pin land in this pool. Existing keys are not moved.',
        groupDefaultNone: 'Unset (probe-first)',
        placeUser: 'Put user in this pool',
        placeUserPlaceholder: 'Pick a user',
        placeUserSuccess: 'Moved {user} keys into "{pool}"',
        placeUserFailed: 'Failed to place the user',
        saveGroupDefaultFailed: 'Failed to save the group default pool',
        clearUserPin: 'Unpin',
        clearUserPinSuccess: 'Unpinned {user}; existing keys stay in their current pool',
        clearUserPinFailed: 'Failed to unpin the user',
        mixedKeys: 'Keys are split across pools',
        boundUsers: 'Users in this pool',
        noBoundUsers: 'No users in this pool yet',
        boundKeys: 'Bound keys',
        noBoundKeys: 'No keys are bound to this pool',
        overrideKey: 'Single-key override',
        overrideKeyHint: 'Only use this when one key is dirty. Day-to-day placement is by user above.',
        bindKey: 'Bind key to this pool',
        bindKeyPlaceholder: 'Pick a user / key',
        bindKeySuccess: 'Key #{id} bound to "{pool}"',
        bindKeyFailed: 'Failed to bind the key',
        ...
        pickAccounts: 'Pick the accounts backing this sub-pool',
        noGroupAccounts: 'No accounts available in this group',
        accountTaken: 'Taken by another sub-pool',
        accountShared: 'Shared with another sub-pool',
        migrate: 'Migrate clean keys',
        migrateHint: 'Move unsuspected keys to other sub-pools. Suspect keys stay here so the evidence trail is preserved.',
        migrateReasonPlaceholder: 'Reason, e.g. pool_burned',
        suspectKeysPlaceholder: 'Suspect key IDs, comma separated (empty means migrate all)',
        migrateSuccess: 'Migrated {count} keys',
        cool: 'Cool down',
        resume: 'Resume',
        deleteTitle: 'Delete sub-pool',
        deleteMessage: 'After deleting "{name}", its keys fall back to whole-group scheduling. Continue?',
        loadFailed: 'Failed to load sub-pools',
        createFailed: 'Failed to create sub-pool',
        saveFailed: 'Failed to save sub-pool',
        saveAccountsFailed: 'Failed to save sub-pool accounts',
        migrateFailed: 'Migration failed',
        deleteFailed: 'Failed to delete sub-pool'
      },
