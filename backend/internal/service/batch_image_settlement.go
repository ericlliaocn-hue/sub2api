...
func (s *BatchImageSettlementService) recordUsageLog(ctx context.Context, job *BatchImageJob, actualCost float64, requestID string, createdAt time.Time) {
	...
	usageLog := &UsageLog{
		...
		AccountRateVersionID:  job.AccountRateVersionID,
		AccountRateSource:     batchImageOptionalStringPtr(job.AccountRateSource),
		AccountRateSnapshot:   job.AccountRateSnapshot,
