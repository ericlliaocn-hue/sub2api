...
func (BatchImageJob) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("account_rate_version_id").Optional().Nillable().Comment("创建 job 时固定的账号倍率版本 ID（Phase 5）"),
		field.String("account_rate_source").Optional().Nillable().MaxLen(32).Comment("创建 job 时固定的账号倍率来源"),
		field.JSON("account_rate_snapshot", map[string]any{}).Optional().SchemaType(map[string]string{dialect.Postgres: "jsonb"}).Comment("创建 job 时固定的账号倍率快照"),
