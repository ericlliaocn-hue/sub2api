...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	schedulePlatform := mediaPlatform
	if endpoint.IsSeedance() && mediaPlatform != service.PlatformSeedance {
	...
	for {
		...
		if boundLookupAccountID > 0 {
			selection, scheduleDecision, err = h.gatewayService.SelectMediaVideoRequestAccount(
				requestCtx, apiKey.GroupID, sessionHash, boundLookupAccountID, routingModel, schedulePlatform,
			)
		} else {
		...
		if err != nil {
			...
			if len(failedAccountIDs) == 0 {
				cls := classifyNoAccountErrorFromGin(c, h.gatewayService, apiKey, requestModel, routingModel, schedulePlatform)
		...
		if endpoint.IsGenerationRequest() && !endpoint.IsSeedance() && mediaPlatform == service.PlatformGrok {
